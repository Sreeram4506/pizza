  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-mozzarella-100">
      <div className="w-full max-w-md bg-mozzarella-200/60 backdrop-blur rounded-2xl p-10 border border-basil-200 shadow-lg">
        <h2 className="text-2xl font-display font-black text-wood-800 mb-4">Welcome back</h2>
        <p className="text-sm text-wood-600 mb-6">Log in to continue ordering.</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button disabled={loading} className="w-full py-3 bg-tomato-600 text-white font-bold rounded hover:bg-tomato-700 transition-colors shadow-lg">{loading ? 'Signing in...' : 'Sign in'}</button>
        </form>
        <div className="mt-6 text-center">
          <p className="text-wood-600 text-sm">
            Don't have an account?{' '}
            <a href="/register" className="text-tomato-600 hover:text-tomato-700 font-semibold">Sign up</a>
          </p>
