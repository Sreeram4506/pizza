...
export default function MenuPage() {
    ...
    return (
        <div className="h-screen flex flex-col bg-[#FAFAF8] overflow-hidden selection:bg-ember-500/15 selection:text-[#1A1410] font-sans">
            ...
            <div className="flex-shrink-0 bg-white/60 backdrop-blur-xl border-b border-[rgba(26,20,16,0.06)] px-4 sm:px-12 py-4 sm:py-6 flex flex-wrap items-center gap-3 sm:gap-5">
                <div className="flex-1 min-w-[200px]">
                    <h1 className="font-display italic font-bold text-lg sm:text-2xl text-[#1A1410] tracking-tight">
                        {settings?.restaurantName || 'Pizza Blast'} Menu
                    </h1>
                    <div className="flex items-center gap-2 text-xs sm:text-sm text-[#5C554E] mt-1">
                        ...
                        <span className="text-[#9B8D74]">·</span>
                        <span className="flex items-center gap-1.5 text-green-700 font-semibold">
                            <span className="w-1.5 h-1.5 rounded-full bg-green-600" /> Open now
                        </span>
                    </div>
                </div>

                <div className="flex bg-[#F5F3EF] rounded-full p-1 border border-[rgba(26,20,16,0.04)]">
                    {['pickup', 'delivery'].map(t => (
                        <button
                            key={t}
                            onClick={() => setOrderType(t)}
                            className={`px-5 py-2 rounded-full text-xs sm:text-sm font-semibold capitalize transition-all ${orderType === t ? 'bg-[#1A1410] text-white shadow-sm' : 'text-[#5C554E] hover:text-[#1A1410]'}`}
                        >
                            {t}
                        </button>
                    ))}
                ...
                <div className="flex items-center gap-2 bg-[#F5F3EF] rounded-full px-4 py-2.5 text-xs sm:text-sm font-semibold text-[#1A1410] border border-[rgba(26,20,16,0.04)]">
                    ...
                    <div className="hidden sm:block mb-10 pt-2">
                        <div className="relative group">
                            ...
                            <input
                                type="text"
                                placeholder="Search menu"
                                className="w-full pl-12 pr-4 py-4 bg-white border border-[rgba(26,20,16,0.08)] rounded-2xl text-sm font-medium focus:outline-none focus:ring-4 focus:ring-ember-500/10 transition-all shadow-sm"
                                value={searchQuery}
                                onChange={(e) => setSearchQuery(e.target.value)}
                            />
                        </div>
                        ...
                        {hasSearchResults && !searchQuery && filteredMenuItems(groupedItems['Popular'] || []).length > 0 && (
                            <section className="relative flex flex-col pt-2 sm:pt-4">
                                <div className="flex items-baseline justify-between mb-6 sm:mb-8">
                                    <h2 className="text-xl sm:text-3xl font-display font-black italic text-[#1A1410] tracking-tight">Most ordered</h2>
                                ...
                                <div ref={mostOrderedRef} className="flex gap-4 overflow-x-auto scrollbar-hide pb-2 -mx-1 px-1 scroll-smooth">
                                    {filteredMenuItems(groupedItems['Popular'] || []).map((item) => (
                                        ...
                                        >
                                            <div className="relative aspect-square rounded-2xl overflow-hidden bg-[#F5F3EF] border border-white shadow-sm">
                                                <img
                                            ...
                                            <p className="mt-3 font-semibold text-[#1A1410] text-sm sm:text-base truncate">{item.name}</p>
                                            <p className="text-[#5C554E] text-sm">${item.price?.toFixed(2)}</p>
                                        </div>
                                    ))}
                                </div>
                            </section>
