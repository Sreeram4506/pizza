    ...
    <div style="font-family: 'Segoe UI', Arial, sans-serif; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #eee; border-radius: 20px; overflow: hidden;">
      ...
      : order.type === 'pickup'
        ...
        <div style="text-align: center; margin-top: 40px;">
          <a href="${frontendUrl}/track/${order.orderNumber}" style="background-color: #dc2626; color: #fff; text-decoration: none; padding: 15px 30px; border-radius: 50px; font-weight: bold; box-shadow: 0 4px 15px rgba(220, 38, 38, 0.2);">Track Your Delivery</a>
        </div>
      </div>
...
export const sendAdminNotification = async (order) => {
  ...
  const emailHtml = `
    <div style="font-family: sans-serif; padding: 20px;">
      ...
      <a href="${frontendUrl}/admin/orders" style="display:inline-block; margin-top: 20px; padding: 15px 25px; background: #dc2626; color: #fff; text-decoration: none; border-radius: 10px; font-weight: bold;">Manage Order</a>
    </div>
  `
  ...
  if (template === 'promotion' || template === 'flash') {
    contentHtml = `
      ...
      <div style="padding: 40px; background-color: #fff; text-align: center;">
        ...
        <a href="${frontendUrl}/menu" style="display: inline-block; background-color: #dc2626; color: #fff; text-decoration: none; padding: 20px 40px; border-radius: 12px; font-weight: 900; text-transform: uppercase; letter-spacing: 1px; font-size: 18px;">CLAIM MY PIZZA NOW</a>
      </div>
    `
    ...
    contentHtml = `
      ...
      <div style="padding: 40px; background-color: #fff;">
        ...
        <div style="margin-top: 30px; border-top: 1px solid #eee; padding-top: 20px;">
          <a href="${frontendUrl}" style="color: #dc2626; font-weight: bold; text-decoration: none;">Read more on our blog →</a>
        </div>
      </div>
    `
  } else {
    ...
    contentHtml = `
      ...
      <div style="padding: 40px; background-color: #fff;">
        ...
        <div style="text-align: center; margin-top: 50px;">
          <a href="${frontendUrl}/menu" style="background-color: #dc2626; color: #fff; text-decoration: none; padding: 18px 35px; border-radius: 50px; font-weight: bold; box-shadow: 0 4px 15px rgba(220, 38, 38, 0.3); font-size: 16px;">Order Your Favorite Pizza</a>
        </div>
      </div>
    `
  }
